...
`environment/city-expressway-v2.jpg` is the cinematic city-and-road environment plate, generated from the approved traffic concept with the prompt in `scripts/bastrop37-environment-prompt.txt`. The Canvas renderer aligns projection to its horizon and adds only animated lane guides, edge lights, gameplay effects, and moving hazards over the detailed art.
